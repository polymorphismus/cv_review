...
class CVRewriteState(BaseModel):
    ...
    original_cv_folder_path: str = Field(
        description="Path to the folder containing original cv"
    )
    output_dir: Optional[str] = Field(
        default=None, description="Folder to save generated artifacts (docx)"
    )
    # Original content
    ...
    matched_skills: List[str] = Field(
        description="Skills candidate has that JD requires"
    )
    partial_skill_matches: List[str] = Field(
        description="Related skills that should be reworded"
    )
    matched_keywords: List[str] = Field(description="ATS-critical keywords found in CV")
    missing_keywords: List[str] = Field(
        description="Keywords missing (DO NOT fabricate)"
    )

    # Requirements alignment
    must_haves_satisfied: List[str] = Field(
        description="Must-have requirements met - highlight these"
    )
    must_haves_missing: List[str] = Field(
        description="Must-haves missing - acknowledge gaps"
    )
    nice_to_haves_satisfied: List[str] = Field(description="Bonus requirements met")

    # Experience relevance
    recent_relevant_experience: List[str] = Field(
        description="Skills/roles from last 2-3 years to prioritize"
    )
    matched_domains: List[str] = Field(
        description="Relevant industry experience to emphasize"
    )
    transferable_domains: List[str] = Field(
        description="Adjacent experience that transfers well"
    )
    ...
    red_flags: List[str] = Field(
        description="Critical issues to address or acknowledge"
    )
    ...
    focus_areas: List[str] = Field(
        description="What to emphasize most in rewrite", default_factory=list
    )

    user_feedback: Optional[str] = Field(
        default=None, description="Feedback user gave to update the created cv"
    )
