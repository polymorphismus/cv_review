...
class QualificationMatchResult(BaseModel):
    ...
    projects_highlight: List[str] = Field(description="Top 2-3 most relevant projects")
...
class RecencyRelevanceResult(BaseModel):
    ...
    continuous_growth: bool = Field(description="Has candidate kept learning?")
...
class WordingMatchResult(BaseModel):
    score: float = Field(description="Match score from 0-100")
    soft_skills_matched: List[str] = Field(description="Soft skills that align")
    soft_skills_missing: List[str] = Field(description="Required soft skills not evident")
    culture_alignment: str = Field(description="strong/moderate/weak")
    communication_style: str = Field(description="professional/casual/technical/balanced")
    value_alignment: List[str] = Field(description="Shared values detected")
    reasoning: str = Field(description="Assessment of cultural and communication fit")
    red_flags: List[str] = Field(description="Cultural misalignment concerns")
