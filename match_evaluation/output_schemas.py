...
class SkillsMatchResult(BaseModel):
    ...
    partial_matches: List[str] = Field(
        description="Related but not exact skill matches"
    )
...
class QualificationMatchResult(BaseModel):
    ...
    matched_items: List[str] = Field(
        description="Education/certs that meet requirements"
    )
    ...
    portfolio_boost: float = Field(
        description="Points added for strong portfolio (0-20)"
    )
...
class SeniorityMatchResult(BaseModel):
    ...
    years_gap: float = Field(
        description="Positive if overqualified, negative if underqualified"
    )
    title_alignment: str = Field(
        description="well-aligned/slight-mismatch/significant-mismatch"
    )
...
class DomainMatchResult(BaseModel):
    ...
    domain_diversity: int = Field(
        description="Number of different domains candidate has worked in"
    )


class RequirementsCoverageResult(BaseModel):
    score: float = Field(
        description="Match score from 0-100+, can exceed 100 with nice-to-haves"
    )
    ...
    nice_to_have_satisfied: List[str] = Field(
        description="Nice-to-have requirements met"
    )
...
class RecencyRelevanceResult(BaseModel):
    score: float = Field(description="Match score from 0-100")
    recent_relevant_experience: List[str] = Field(
        description="Skills/roles from last 2-3 years"
    )
    outdated_experience: List[str] = Field(description="Skills not used recently")
    most_recent_role_match: str = Field(
        description="How well does latest role align with job?"
    )
...
class KeywordMatchResult(BaseModel):
    score: float = Field(
        ge=0, le=100, description="Match score from 0-100 based on keyword presence"
    )
    matched_keywords: List[str] = Field(
        description="Critical keywords found in CV (exact matches)"
    )
    missing_keywords: List[str] = Field(description="Critical keywords NOT found in CV")
    reasoning: str = Field(
        description="Explanation of keyword match results and ATS implications"
    )
    red_flags: List[str] = Field(
        description="Critical keywords missing that could cause ATS rejection"
    )
    keyword_frequency: dict = Field(
        description="How many times each matched keyword appears, e.g. {'Python': 5, 'AWS': 3}"
    )


class FinalScoringResult(BaseModel):
    decision: Literal[
        "Strong Match", "Good Match", "Partial Match", "Weak Match", "Poor Match"
    ] = Field(description="Strong Match/Good Match/Partial Match/Weak Match/Poor Match")
    ...
    recommendation: str = Field(
        description="Detailed 2-3 sentence recommendation on wheather it is worth for candidate to apply to this role or not"
    )
    focus_areas: List[str] = Field(
        description="What the candidate need to work on in order to match the required position"
    )
...
class WeightingStrategy(BaseModel):
    ...
    skills_match: float = Field(
        ge=0, le=1, description="Weight for technical skills matching"
    )
    keyword_match: float = Field(
        ge=0, le=1, description="Weight for ATS keyword matching"
    )
    requirements_coverage: float = Field(
        ge=0, le=1, description="Weight for must-have requirements"
    )
    seniority_match: float = Field(
        ge=0, le=1, description="Weight for experience level"
    )
    qualification_match: float = Field(
        ge=0, le=1, description="Weight for education/certs/portfolio"
    )
    recency_relevance: float = Field(
        ge=0, le=1, description="Weight for how current experience is"
    )
    domain_match: float = Field(
        ge=0, le=1, description="Weight for industry experience"
    )

    reasoning: str = Field(description="Why these weights were chosen for this role")
    role_archetype: str = Field(
        description="E.g., 'senior-technical', 'entry-level', 'domain-specialist', 'leadership'"
    )
